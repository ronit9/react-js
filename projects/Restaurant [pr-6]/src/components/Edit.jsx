...
const Add = () => {
  ...
  const { id } = useParams(); // URL params માંથી id લઉં
  ...
  const recId = allrecipes.find((recipe) => recipe.id == id);
  useEffect(() => {
    if (recId) {
      setRecipes({
        recipeName: recId.recipeName,
        info: recId.info,
        ingredients: recId.ingredients,
        price: recId.price,
        image: recId.image,
        category: recId.category,
        taste: recId.taste,
        critaria: recId.critaria,
      });
    }
  }, [recId]);
  const formSubmit = (e) => {
    ...
    let rec = allrecipes.map((item) => {
      if (item.id == id) {
        return {
          ...recipes, // current form state નું data
          id: id, // old id maintain
        };
      } else {
        return item;
      }
                  ...
                  <Card.Body className="m-3 d-block">
                    ...
                    <Card.Text className="d-flex flex-wrap  align-items-center">
                      ...
                      {recipes.taste.map((item) => {
                        return (
                          <span className="bg-danger taste mb-2">{item}</span>
                        );
                      })}
                    </Card.Text>
                  </Card.Body>
