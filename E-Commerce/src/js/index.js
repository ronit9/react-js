...
const filter = (type) => {
    if (type === "Clothing") {
        let clothes = product.filter((val) => val.category === "clothing")
        showproduct(clothes)
    } else if (type === "beauty") {
        let beauty = product.filter((val) => val.category === "beauty")
        showproduct(beauty)
    } else if (type === "sports") {
        let sports = product.filter((val) => val.category === "sports")
        showproduct(sports)
    } else if (type === "furniture") {
        let furniture = product.filter((val) => val.category === "furniture")
        showproduct(furniture)
    } else if (type === "groceries") {
        let groceries = product.filter((val) => val.category === "groceries")
        showproduct(groceries)
    } else if (type === "electronics") {
        let electronics = product.filter((val) => val.category === "electronics")
...
const showproduct = (getitem) => {
    ...
    <div class="flip-card-inner">
        <div class="flip-card-front">
            <img src="${val.image}" style="animation-delay: ${index * 0.3}s;" class="card-img-top" alt="...">
        </div>
...
<button class="cartBtn" type="button" onclick="addtocart(${val.id} )">
...
const addtocart = (id) => {
    const products = product.find((val) => val.id === id)
    cart.push(products)
